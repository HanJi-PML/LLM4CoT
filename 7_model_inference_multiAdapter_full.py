...
def main():
    ...
    from utils_new import split_dataset, sequencial_inference_multiAdapter_fullModels, load_models_once

    full_models = [
        {'adapter_name': 'multiAdapter_task1',
        'adapter_path': '/data/LLM_indoor/LLaMA-Factory-main/saves/LLM_Multitask/Dataset3_12k_10K_task1-8B-full-20260106-1107AM'
    ...
    # 一次性加载所有多适配器模型到不同GPU
    full_model_paths = [model['adapter_path'] for model in full_models]
    try:
        load_models_once(full_model_paths, gpu_ids=[0, 1])
    ...
    for i in range(len(test_datasets)):
        ...
        # ====== 基本参数 ======
        parser.add_argument('--model_name_or_path', default=full_models)
        ...
        parser.add_argument('--vllm_gpu_memory_utilization', default=0.3, type=float)
        ...
        # 多适配器并行推理
        sequencial_inference_multiAdapter_fullModels(args, used_datasets, test_samples)
