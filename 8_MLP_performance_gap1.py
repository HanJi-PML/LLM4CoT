"""
MLP Performance Gap Analysis Script
====================================
本脚本用于测试三个MLP模型的级联推理性能：
...
def test_single_dataset(config, device):
    ...
    # ========== 关键修复：从数据集中提取真实的 pos_min/pos_max ==========
    ...
    for idx, sample in enumerate(samples):
        try:
            ...
            pred_aps = atcnn_predict_all_users(
                atcnn_model, pred_sinr, rate_vector, max_ue, ap_num, device
            )
            ...
            # Task 3: GNN预测资源分配
            graph_data = create_graph_for_inference(pred_sinr, rate_vector, pred_aps, ap_num)
            ...
            pred_throughput = throughput_cal(rate_vector, pred_aps, pred_sinr, pred_rho)
...
def main():
    ...
    if all_results:
        ...
        summary_path = f'{SAVE_FOLDER}/MLP_performance_gap_results_{timestamp}.csv'
