"""
MLP Performance Gap Analysis Script (真实SINR + 高斯扰动版本)
============================================================
本脚本用于测试模型级联推理性能（使用真实SINR+高斯扰动代替DNN预测）：
- Task 1: 使用真实SINR + 1dB MAE高斯扰动（不使用DNN）
- Task 2: ATCNN 进行 AP 选择预测
- Task 3: GNN 进行功率分配预测
...
from ATCNN_model import ATCNN, switch  # type: ignore
...
# SINR高斯扰动参数
# 高斯噪声 N(0, σ) 的 MAE = σ * sqrt(2/π) ≈ 0.798 * σ
# 若要 MAE = 1dB，则 σ = 1 / sqrt(2/π) ≈ 1.253 dB
SINR_NOISE_MAE = 0  # 目标MAE (dB)
SINR_NOISE_STD = SINR_NOISE_MAE / np.sqrt(2 / np.pi)  # 约1.253 dB
...
# ==================== 4. SINR扰动函数 ====================
def add_gaussian_noise_to_sinr(sinr_matrix, target_mae=1.0):
    """
    对SINR矩阵添加高斯噪声
    
    参数:
        sinr_matrix: 真实SINR矩阵 (UE_num x AP_num)
        target_mae: 目标MAE (dB)，默认为1.0 dB
    ...
    说明:
        ...
        若要 MAE = 1dB，则 σ = 1 / sqrt(2/π) ≈ 1.253 dB
    """
    noise_std = target_mae / np.sqrt(2 / np.pi)
    noise = np.random.normal(0, noise_std, size=sinr_matrix.shape)
    noisy_sinr = sinr_matrix + noise
    return noisy_sinr
...
# ==================== 6. ATCNN 推理辅助函数 ====================
def mirror_input(input_data, num_users, max_ue, ap_size):
    """ATCNN的mirror函数"""
    M = max_ue
    AP_num = ap_size
    UE_num = num_users
    ...
    return np.array(mirroring_input, dtype=np.float32)


def atcnn_predict_all_users(model, sinr_matrix, rate_vector, max_ue, ap_size, device):
    """使用ATCNN预测所有用户的AP选择"""
    ...
    for i in range(num_users):
        sinr_norm = (sinr_matrix[i] - SNR_MIN) / (SNR_MAX - SNR_MIN)
        sinr_norm = np.clip(sinr_norm, 0, 1)
        rate_norm = (rate_vector[i] - R_MIN) / (R_MAX - R_MIN)
        rate_norm = np.clip(rate_norm, 0, 1)
        user_data = list(sinr_norm) + [rate_norm]
        input_data.extend(user_data)
    
    mirrored_input = mirror_input(input_data, num_users, max_ue, ap_size)
    ...
    with torch.no_grad():
        for ue_idx in range(num_users):
            target_start = ue_idx * user_dim
            target_end = (ue_idx + 1) * user_dim
            
            condition_list = switch(mirrored_input.copy(), ue_idx, user_dim)
            
            Target = torch.tensor(mirrored_input[target_start:target_end], dtype=torch.float32).unsqueeze(0).to(device)
            Condition = torch.tensor(condition_list[0:], dtype=torch.float32).unsqueeze(0).to(device)
            
            output = model(Target, Condition)
            pred_ap = output.argmax(dim=1).item() + 1
...
def test_single_dataset(config, device):
    ...
    if not os.path.exists(dataset_path):
        ...
        return None
    if not os.path.exists(atcnn_model_path):
        print(f"  ❌ ATCNN模型文件不存在: {atcnn_model_path}")
        return None
    ...
    # 加载模型（不需要DNN，只需要ATCNN和GNN）
    print("  加载模型...")
    
    # ATCNN
    user_dim = ap_num + 1
    cond_dim = user_dim * max_ue
    atcnn_model = ATCNN(
        input_dim=user_dim,
        cond_dim=cond_dim,
        cond_out_dim=user_dim,
        output_dim=ap_num
    )
    atcnn_model.load_state_dict(torch.load(atcnn_model_path, map_location=device))
    atcnn_model.to(device)
    atcnn_model.eval()
    ...
    for idx, sample in enumerate(samples):
        try:
            ...
            # Task 1: 使用真实SINR + 高斯扰动（替代DNN预测）
            noisy_sinr = add_gaussian_noise_to_sinr(gt_sinr, target_mae=SINR_NOISE_MAE)
            
            # 记录实际的SINR扰动MAE
            sinr_mae = np.abs(noisy_sinr - gt_sinr).mean()
            all_sinr_mae.append(sinr_mae)
            
            # Task 2: ATCNN预测AP选择（使用扰动后的SINR）
            pred_aps = atcnn_predict_all_users(
                atcnn_model, noisy_sinr, rate_vector, max_ue, ap_num, device
            )
            
            atcnn_acc = np.mean(pred_aps == gt_aps)
            all_atcnn_acc.append(atcnn_acc)
            
            # Task 3: GNN预测资源分配（使用扰动后的SINR和预测的AP选择）
            graph_data = create_graph_for_inference(noisy_sinr, rate_vector, pred_aps, ap_num)
            ...
            # 计算吞吐量（使用扰动后的SINR计算预测吞吐量）
            gt_rho = normalize_rho(gt_aps, gt_ra, ap_num, ue_num)
            gt_throughput = throughput_cal(rate_vector, gt_aps, gt_sinr, gt_rho)
            
            pred_rho = normalize_rho(pred_aps, pred_ra, ap_num, ue_num)
            # 注意：预测吞吐量使用扰动后的SINR（noisy_sinr），模拟实际场景中Task1输出的影响
            pred_throughput = throughput_cal(rate_vector, pred_aps, noisy_sinr, pred_rho)
    ...
    result = {
        ...
        'max_UE': max_ue,
        'samples': valid_count,
        'SINR_noise_target_MAE': SINR_NOISE_MAE,
        'SINR_actual_MAE': np.mean(all_sinr_mae),
        'ATCNN_Accuracy': np.mean(all_atcnn_acc),
    ...
    print(f"\n  结果汇总:")
    print(f"    [Task1] SINR扰动目标MAE:   {result['SINR_noise_target_MAE']:.4f} dB")
    print(f"    [Task1] SINR扰动实际MAE:   {result['SINR_actual_MAE']:.4f} dB")
    print(f"    [Task2] ATCNN Accuracy:   {result['ATCNN_Accuracy']*100:.2f}%")
...
def main():
    ...
    print("使用真实SINR + 高斯扰动（替代DNN预测）")
    print(f"目标SINR扰动: MAE={SINR_NOISE_MAE}dB, σ={SINR_NOISE_STD:.3f}dB")
    print("=" * 80)
    ...
    if all_results:
        ...
        summary_path = f'{SAVE_FOLDER}/MLP_performance_gap_noisy_SINR_{SINR_NOISE_MAE}dB_{timestamp}.csv'
        ...
        print("总体平均:")
        print("-" * 60)
        print(f"  SINR扰动目标MAE:    {SINR_NOISE_MAE:.4f} dB")
        print(f"  SINR扰动实际MAE:    {summary_df['SINR_actual_MAE'].mean():.4f} dB")
        print(f"  ATCNN Accuracy:     {summary_df['ATCNN_Accuracy'].mean()*100:.2f}%")
